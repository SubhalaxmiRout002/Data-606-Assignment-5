...
hist(sample_means50)
paste0("Sample mean : ",round(mean(sample_means50),2))
population_mean <- round(mean(price),2)
paste0("Population mean : ",population_mean)

```

 The distribution looks a normal distribution. 
 Mean home price based on the sample is 180873.43. 
 The population mean of home price is 180796.06.
...
paste0("Sample mean : ",round(mean(sample_means150),2))
```

 The shape of the distribution with sample size 150 looks normal distribution. Shape is bell shape and unimodal. 
 
After comparing both histogram can observe sample_mean150 has narrow spread than sample_mean50.

Based on the sampling distribution mean sale price of homes in Ames is 180856.22.
 ...
 From the sampling distribution 2 and 3, the sampling distribution sample size 150 has a smaller spread than sample size 50. If we are concerned with making estimates that are more often close to the true value, we would prefer a distribution with a small spread because it is more representative of the overall population.
